...
ここから下は日本語で書く。語彙の選択そのものが判断の対象になるため。

上のセクションが「こう作りたい」という宣言で、ここは実際に作りながら
確定した規則。**決めた経緯と却下した案は `DECISIONS.md`** に置いてある。
規則の背景を知りたいとき、あるいは「これは検討済みか」を確かめたいときは
そちらを見る。
...
- リンクは赤で書く。通常の文字色にすると押せることが分からなくなる
- ホバーで赤くなる（`.link-accent`）のは、操作への反応として正しい
- 塗りつぶしの赤いボタンは、その画面で最も重要な1つだけ
- Contact のチェックマークは赤のまま。**このサイトで赤は警告色ではなく
  プライマリ**なので、肯定的な意味で使ってよい。緑を足すのは
  「One accent」に反する
...
同じセクション名でも、置かれる場所で役割が違う。役割が違えば書き方も違う。

| | 役割 | 書き方 |
|---|---|---|
| セクション見出し | コンテンツのラベル | 英語（EXPERIENCE） |
| Rail（1280px 以上） | いまどこにいるか | 英語。見出しと対応させる |
| メニュー（1280px 未満） | どこへ行くか選ぶ | ひらがな。コマンド窓だから |

メニューは「これまで / きかれたこと / はなしたこと / かいたもの /
できること / はなしかける」。**漢字の名詞（経歴・取材・登壇）は資料の
目次の語**で、並べると会社案内のナビになる。5つを「〜たこと」で揃えた。
**桁を揃えるより語形を揃えるほうが効く。**
...
にして内側だけをスクロールさせる。枠は**背景と穴の関係**なので動かない。
ビューポートに固定したオーバーレイで描くと、スクロールバーの出入り・
モバイルの URL バー・巨大な影の再描画でずれる。
...
採用されたもの（Menu、HUD、Rail、SectionMenu、年フィルタ、枠取り）は
全部そうで、却下されたもの（「◯◯があらわれた！」、ぼうけんのしょ、
スキルのゲージ表示）は全部そうでなかった。
...
足りないアンカーを1つだけ足す。密度は 12px / 14px 主体のまま保つ。

### コントラストは測ってから決める

`--color-text-muted` は 12px で40箇所以上使われている。**一番小さい文字が
一番読みにくい色**にならないよう、WCAG AA（4.5）を背景と
サーフェスの両方で満たす値を選ぶ。現在値 `#6E6C64` は 5.00 / 4.58。

### 実績リストは全件 DOM に出す

「もっと見る」で隠す項目も、DOM には全件置いて `hidden` 属性で折りたたむ。
`slice()` で間引くとクローラーやエージェントから見えなくなる。
年フィルタで絞る場合も同じ。
