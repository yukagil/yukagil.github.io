...
ここから下は、実際に作りながら判断したことの記録。
上のセクションが「こう作りたい」の宣言で、ここは「こう作った、なぜなら」。
文書は日本語で書く。語彙の選択そのものが判断の対象になるため。

**原則: すべてのデザインには意図があるべき。説明できない、あるいは
ロジックが破綻しているものは再整理の対象。**

---

### 赤は「押せる」の合図

`--color-accent` が付いているものは、押すと何かが起きる。
静的な装飾として赤を置かない。

- リンクは通常の文字色にせず、赤で書く。クリックできることが分からなくなる
- ホバーで赤くなる（`.link-accent`）のは、操作への反応として正しい
- 塗りつぶしの赤いボタンは、その画面で最も重要な1つだけ（HUD の Contact、フォームの送信）
- Contact のチェックマークは赤のまま。**このサイトで赤は警告色ではなく
  プライマリなので**、肯定的な意味で使ってよい。緑を足すのは
  「One accent」に反する

セクションを開いたとき、塗りつぶしの赤が2つ以上見えていたら疑う。

### 破線はステータス画面の中だけ

破線（`1px dashed`）はカード裏の RPG ステータス画面の語彙。
CLASS とゲージと STRENGTHS を仕切るためのもの。

**それ以外の区切りは 1px の実線。** 破線を見たら「ステータス画面の中だ」
と分かる状態を保つ。区切り線に2種類あって使い分けの説明がつかないなら、
それは意味を持っていないということ。

### 移動の言葉は、役割で書き分ける

同じ「セクション名」でも、置かれる場所で役割が違う。役割が違えば
書き方も違ってよい。

| | 役割 | 書き方 |
|---|---|---|
| セクション見出し | コンテンツのラベル | 英語（EXPERIENCE） |
| Rail（広い画面） | いまどこにいるか | 英語。見出しと対応させる必要がある |
| メニュー（狭い画面） | どこへ行くか選ぶ | ひらがな。コマンド窓だから |

メニューは「これまで / きかれたこと / はなしたこと / かいたもの /
できること / はなしかける」。

- **漢字の名詞（経歴・取材・登壇）は資料の目次の語**。並べると会社案内の
  ナビになる。ゲームのメニューに英語が並ばないのと同じ理由で、ここは開く
- 5つを「〜たこと」で揃えた。**桁を揃えるより語形を揃えるほうが効く**
- 「はなしかける」は言葉遊びではない。RPG のそのコマンドが指す動作と、
  このセクションの用途が同じ
- 英語との併記はしない。短さが本体のパネルで単語数が倍になり、
  2文字の桁に押し込むために言葉が歪む（「連絡」はあの列以外で誰も書かない）

### 枠は描かない、背景として置く

ページ全体を機器の画面に見立て、周囲を筐体として残す。

実装は `html` の背景色 + `#root` を `position: fixed; inset: var(--frame)`
にして**内側だけをスクロールさせる**。枠は「描いた線」ではなく
**背景と穴の関係**なので、原理的に動かない。

かつてビューポート固定のオーバーレイ（`box-shadow: 0 0 0 100vmax`）で
...
**固定要素（HUD・メニュー）も枠の内側に入れる。** 外に出ると筐体の上に
UI が乗ることになり、「画面の中」に見えなくなる。

### 遊びは衣装ではなく機構

このサイトの面白さは「操作すると世界が変わる」ことにある。
見た目だけゲームを借りたものは機能しない。

- 通ったもの: Menu、HUD、Rail、SectionMenu、年フィルタ、枠取り
  — **全部「操作できるもの」**
- 通らなかったもの: 「◯◯があらわれた！」、ぼうけんのしょ、
  スキルをゲージで表示、経歴を「なかま」欄に、セクション遷移時のフラッシュ
  — **全部「見た目を借りただけのもの」**

新しい遊びを足すときは、まず「これは操作できるか」を問う。

### 文字サイズは 12px をメタ情報に留める

- **12px**: 日付、媒体名、チップ、必須マーク。それ以外に使わない
- **14px**: 本文、説明、サマリ
- **16px**: フォーム入力欄（iOS Safari は 16px 未満でフォーカスすると
  ページを自動ズームする）
- **18px**: セクション見出し

階層が足りないと感じても、**全体を一段ずつ上げてはいけない**。比率が
変わらないまま絶対値だけ大きくなり、120% 拡大したように見える。
足りないアンカーを1つだけ足す。

密度は 12px / 14px 主体のまま保つ。ミニマルさはそこにある。

### コントラストは測ってから決める

`--color-text-muted` は 12px で 40箇所以上使われている。
**一番小さい文字が一番読みにくい色**にならないよう、WCAG AA（4.5）を
背景（`--color-bg`）とサーフェス（`--color-surface`）の両方で満たす値を選ぶ。

現在値 `#6E6C64` は 5.00 / 4.58。

### 実績リストは全件 DOM に出す

「もっと見る」で隠す項目も、**DOM には全件置いて `hidden` 属性で
折りたたむ**。`slice()` で間引くとクローラーやエージェントから見えなくなる。
年フィルタで絞る場合も同じ。
...
**構成・情報設計**
Timeline（4セクションを1本の年表に統合）/ Themes（形式でなく主題で束ねる）/
Ask（「何に困っていますか」から入る）/ Thesis（主張を先に置く）/
Reverse（Services を先頭に）/ Canon（代表作3本）/ Merge（Interviews を
Speaking に統合）
...
Anchor（行ごとのリンク）/ Material（資料ありで絞る）/ UrlState（絞り込みを
URL に）/ Top（先頭へ戻る）/ Nav（HUD にセクション一覧）/ Crumb（HUD に
現在地名）/ Articles（note 18本を一覧表示）/ Thumbs（行からサムネイル）/
Output（経歴に発信件数）/ AllOpen（最初から全件）/ Sticky（見出しを貼り付け）/
Cue（▼ scroll）

**演出**
Encounter（「◯◯があらわれた！」）/ Save（ぼうけんのしょ）/
LevelUp（読み進むと年が上がる）/ Skills（主題をゲージで）/ Party（経歴を
なかま欄に）/ Command（Contact をコマンド窓に）/ Wipe（遷移時のフラッシュ）/
Shrink（カードが縮む）

**廃止したもの**
判子（`.hanko`）— 「唯一記憶に残る要素」として仕様に書かれていたが、
一度も描画されないまま CSS だけが残っていた。仕様が「手描き SVG でないと
意味がない」と自ら述べており、コードで再現する前提と噛み合わない
